              ...
              <div 
                ...
                <div class="approval-details">
                  <div class="detail-item">
                    <strong>评审名称:</strong>
                    <span class="review-name">{{ getReviewName(approval.review) }}</span>
                  </div>
                  <div class="detail-item">
                    <strong>审批状态:</strong>
                    <el-tag 
  ...
  setup(props) {
    ...
    const loadApprovalHistory = async () => {
      ...
      try {
        ...
        const endpoint = `/api/versions/jarvis/${encodeURIComponent(versionId)}/approval-statuses`
    ...
    const getReviewName = (review) => {
      ...
      // 如果API已经返回了名称，直接使用
      if (review.name && review.name !== review.id) {
        return review.name
      }
      
      // 尝试从缓存获取
      const cachedReview = reviewsCacheManager.getReviewById(review.id)
      if (cachedReview && cachedReview.name) {
        console.log(`使用缓存的评审名称: ${review.id} -> ${cachedReview.name}`)
        return cachedReview.name
      }
      
      // 如果缓存中没有，返回ID
      console.warn(`未找到评审名称，使用ID: ${review.id}`)
      return review.id
    }
