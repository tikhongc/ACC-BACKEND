  ...
  <div class="workflow-diagram">
            ...
            <div class="step-info">
              ...
              <div v-if="step.duration" class="step-duration">
                <el-tag size="small" type="info">
                  {{ step.duration }} {{ step.dueDateType === 'CALENDAR_DAY' ? 'Calendar Days' : 'Business Days' }}
                </el-tag>
              </div>
              ...
              <div class="step-candidates">
                ...
                <div v-if="step.candidates?.users?.length > 0" class="candidates-section">
                  <div class="candidates-label">👤 Users:</div>
                ...
                <div v-if="step.candidates?.roles?.length > 0" class="candidates-section">
                  <div class="candidates-label">🏷️ Roles:</div>
                  <div class="candidates-list">
                    <el-tag 
                ...
                <div v-if="step.candidates?.companies?.length > 0" class="candidates-section">
                  <div class="candidates-label">🏢 Companies:</div>
                  <div class="candidates-list">
                    <el-tag 
              ...
              <div v-if="step.groupReview?.enabled" class="group-review">
                <el-tag size="small" type="warning">
                  Group Review: {{ step.groupReview.type }} 
                  {{ step.groupReview.min ? `(Min ${step.groupReview.min} people)` : '' }}
                </el-tag>
              </div>
            </div>
          </div>
      ...
      <div class="options-grid">
        <div 
          ...
          <div class="option-info">
            ...
            <el-tag v-if="option.builtIn" size="small" type="info">Built-in</el-tag>
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="config-grid">
        ...
        <div v-if="workflow.copyFilesOptions?.enabled" class="config-card">
          <div class="config-header">
            ...
            <span>File Copy Configuration</span>
          </div>
          <div class="config-content">
            <div class="config-item">
              <strong>Allow Override:</strong> {{ workflow.copyFilesOptions.allowOverride ? 'Yes' : 'No' }}
            </div>
            <div class="config-item">
              <strong>Condition:</strong> {{ workflow.copyFilesOptions.condition }}
            </div>
            <div class="config-item">
              <strong>Include Markups:</strong> {{ workflow.copyFilesOptions.includeMarkups ? 'Yes' : 'No' }}
            </div>
          </div>
        </div>
        ...
        <div class="config-card">
          ...
          <div class="config-content">
            <div class="config-item">
              <strong>Allow Initiator to Edit:</strong> 
              {{ workflow.additionalOptions?.allowInitiatorToEdit ? 'Yes' : 'No' }}
            </div>
            <div class="config-item">
              <strong>Attached Attributes:</strong> 
              {{ workflow.attachedAttributes?.length || 0 }} items
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="workflow-timeline">
      <div class="timeline-item">
        <strong>Created At:</strong> {{ formatDate(workflow.createdAt) }}
      </div>
      <div class="timeline-item">
        <strong>Updated At:</strong> {{ formatDate(workflow.updatedAt) }}
      </div>
    </div>
  </div>
</template>
